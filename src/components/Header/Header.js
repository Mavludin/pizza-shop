...
export const Header = ({ totCount, clothes, accessories }) => {
      ...
      setTimeout(
        () =>
          Scroll.animateScroll.scrollTo(
            parseInt(clothes.current.offsetTop - 90)
          ),
        500
      );
    } else
      Scroll.animateScroll.scrollTo(parseInt(clothes.current.offsetTop - 90));
  };
  ...
  const scrollToAccessories = () => {
    if (location.pathname !== "/") {
      ...
      setTimeout(
        () =>
          Scroll.animateScroll.scrollTo(
            parseInt(accessories.current.offsetTop - 90)
          ),
        500
      );
    } else
  ...
  if (mobileMenuFlag)
    mobileMenuClasses = `${classes.MobileMenu} ${classes.ShowMobileMenu}`;
      ...
      <div className={`container ${classes.HeaderWrap}`}>
        ...
        <div className={classes.HeaderLeft}>
          ...
          <nav className={classes.TopMenu}>
            <ul>
              <li>
                <Link onClick={scrollToClothing} to="/">
                    Go to pizza
                </Link>
              </li>
              <li>
                <Link onClick={scrollToAccessories} to="/">
                  Snacks
                </Link>
              </li>
            </ul>
